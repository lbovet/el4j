 ...
 */
package ch.elca.el4j.tests.util.metadata.attributes.helper;
...
import ch.elca.el4j.util.metadata.attributes.AttributeSourceAware;
import ch.elca.el4j.util.metadata.attributes.GenericAttributeSource;
...
public class ExampleInterceptor implements MethodInterceptor,
    ...
    public Object invoke(MethodInvocation methodInvocation) throws Throwable {
        ...
        Object attributes = m_attributeSource.getAttribute(methodInvocation.getMethod(), 
                methodInvocation.getThis().getClass());

        if (attributes == null) {
            s_logger.info("Method '" + methodInvocation.getMethod() 
                + "' does not have any attribute.");
        } else if (attributes instanceof ExampleAttributeInterface) {
            att = (ExampleAttributeInterface) attributes;
