 ...
 */
public class LbTestServer3 {

	/**
	 * Private logger.
	 */
	private static Logger s_logger = LoggerFactory.getLogger(LbTestServer3.class);
	...
	public static void main(String args[]) {
		...
		ApplicationContext applicationContext = new ModuleApplicationContext(
			new String[] {"classpath*:mandatory/*.xml",
				"classpath:loadbalancing/server/startup3.xml",
				"classpath:loadbalancing/remoting/rmi-protocol-config.xml",
				"classpath*:loadbalancing/remoting/multiplermi-protocol-config.xml"},
			(String[]) null,
			false,
			(ApplicationContext) null);

		s_logger.debug("Starting up with name ....");

		BusinessObject obj = (BusinessObject)applicationContext.getBean("rmiTestObjImpl3");
