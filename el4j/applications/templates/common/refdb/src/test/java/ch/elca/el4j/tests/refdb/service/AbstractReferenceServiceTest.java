...
public abstract class AbstractReferenceServiceTest extends AbstractTestCaseBase {
	...
	protected ReferenceService getReferenceService() {
		if (m_referenceService == null) {
			m_referenceService = (ReferenceService) getApplicationContext().getBean("referenceService");
		}
	...
	public void testInsertLink() {
		...
		link.setDescription("This page shows you how to develop an " + "application with iBatis.");
		link.setVersion("2.0");
		...
		// The Joda-Time way of creating a date (the created date is: October the 25., 2004)
		link.setDate(new LocalDate(2004, 10, 25));
	...
	/**
	 * This test adds a link and keywords. Some of the keywords are related with the link and some are not. After that
	 * the link will be get by primary key and compared with the added one. At the end the link will be removed.
	 */
	@Test
	public void testInsertGetRemoveLink() {
		...
		keyword4.setDescription("This keyword is not " + "assigned to any reference.");
		keywordDao.saveOrUpdate(keyword);
		keywordDao.saveOrUpdate(keyword2);
		keywordDao.saveOrUpdate(keyword3);
		...
		assertEquals("There are not three keywords related with the link.", 3, listKeywords2.size());

		for (Keyword k : listKeywords2) {
			if (!(k.equals(keyword) || k.equals(keyword2) || k.equals(keyword3))) {
				fail("There was an unexpected keyword related with link.");
			}
		}

		service.deleteReference(link2.getKey());
		List list = service.getReferencesByName("iBatis Data Mapper Developer Guide");
		assertEquals("There is still a link with " + "name 'iBatis Data Mapper Developer Guide'.", list.size(), 0);
	...
	/**
	 * This test adds a link with three keywords. After saving the link the same link will be get from database. This
	 * link get a new name and one keyword will be replaced by another. Afterwards changes will be saved to database.
	 */
	@Test
	public void testInsertChangeLink() {
		...
		List list = service.getReferencesByName("iBatis Data Mapper " + "Developer Guide");
		assertEquals("Not one link with name 'iBatis Data Mapper " + "Developer Guide'.", 1, list.size());
		...
		List<Reference> emptyList = service.getReferencesByName("iBatis Data Mapper Developer Guide");
		assertEquals("Link with name 'iBatis Data Mapper Developer Guide' " + "still exists.", emptyList.size(), 0);

		List<Reference> list2 = service.getReferencesByName("iBatis SqlMap 2.0 " + "Developer Guide");
		assertEquals("Not one link with name 'iBatis SqlMap 2.0 " + "Developer Guide'.", 1, list2.size());
		Link link3 = (Link) list.get(0);
		assertTrue("Links are not equal.", link2.equals(link3));

		Set<Keyword> listKeywords3 = link3.getKeywords();
		assertEquals("There are not three keywords related with the link.", 3, listKeywords3.size());
		for (Keyword k : listKeywords3) {
			if (!(k.equals(keyword) || k.equals(keyword3) || k.equals(keyword5))) {
				fail("There was an unexpected keyword related with link.");
			}
		}
	}
	...
	public void testGetAllLinks() {
		...
		link2.setDescription("This is the best online dictionary " + "for English and German.");
		...
		// link2.setDate(new Date(c2.getTimeInMillis()));

		// The Joda-Time way of creating a date (the created date is: November the 8., 2004)
		link.setDate(new LocalDate(2004, 11, 8));
	...
	public void testInsertFormalPublication() {
		...
		formalPublication.setDescription("This manual shows you how to develop an application with iBatis.");
		...
		// The Joda-Time way of creating a date (the created date is: October the 25., 2004)
		formalPublication.setDate(new LocalDate(2004, 10, 25));
	...
	/**
	 * This test adds a formal publication and keywords. Some of the keywords are related with the formal publication
	 * some are not. After that the formal publication will be get by primary key and compared with the added one. At
	 * the end the formal publication will be removed.
	 */
	@Test
	public void testInsertGetRemoveFormalPublication() {
		...
		formalPublication = (FormalPublication) service.saveReference(formalPublication);

		FormalPublication formalPublication2 = (FormalPublication) service
			.getReferenceByKey(formalPublication.getKey());

		// the next two lines are here due to a strange bug of mysql
		// formalPublication.getWhenInserted().setNanos(0);
		// formalPublication2.getWhenInserted().setNanos(0);

		assertTrue("Formal publications are not equal.", formalPublication.equals(formalPublication2));

		Set<Keyword> listKeywords2 = formalPublication2.getKeywords();
		assertEquals("There are not three keywords related with the formal " + "publication.", 3, listKeywords2.size());
		for (Keyword k : listKeywords2) {
			if (!(k.equals(keyword) || k.equals(keyword2) || k.equals(keyword3))) {
				fail("There was an unexpected keyword related with " + "formal publication.");
			}
		}

		service.deleteReference(formalPublication2.getKey());
		List<Reference> list = service.getReferencesByName("iBatis Data Mapper Developer Guide");
		assertEquals("There is still a formal publication with " + "name 'iBatis Data Mapper Developer Guide'.", list
			.size(), 0);
	}

	/**
	 * This test adds a formal publication with three keywords. After saving the formal publication the same formal
	 * publication will be get from database. This formal publication get a new name and one keyword will be replaced by
	 * another. Afterwards changes will be saved to database.
	 */
	@Test
	public void testInsertChangeFormalPublication() {
		...
		List list = service.getReferencesByName("iBatis Data Mapper Developer Guide");
		assertEquals("Not one formal publication with name 'iBatis Data Mapper " + "Developer Guide'.", 1, list.size());
		FormalPublication formalPublication2 = (FormalPublication) list.get(0);
		...
		formalPublication2 = (FormalPublication) service.saveReference(formalPublication2);

		List<Reference> emptyList = service.getReferencesByName("iBatis Data Mapper Developer Guide");
		assertEquals("Formal publication with name 'iBatis Data Mapper " + "Developer Guide' still exists.", emptyList
			.size(), 0);

		List<Reference> list2 = service.getReferencesByName("iBatis SqlMap 2.0 Developer Guide");
		assertEquals("Not one formal publication with name 'iBatis SqlMap 2.0 " + "Developer Guide'.", 1, list2.size());
		FormalPublication formalPublication3 = (FormalPublication) list.get(0);
		assertTrue("Formal publications are not equal.", formalPublication2.equals(formalPublication3));

		Set<Keyword> listKeywords3 = formalPublication3.getKeywords();
		assertEquals("There are not three keywords related with the formal " + "publication.", 3, listKeywords3.size());
		for (Keyword k : listKeywords3) {
			if (!(k.equals(keyword) || k.equals(keyword3) || k.equals(keyword5))) {
				fail("There was an unexpected keyword related with " + "formal publication.");
			}
	...
	public void testGetAllFormalPublications() {
		...
		formalPublication2.setDescription("This document shows you how to " + "use the spring framework.");
		...
		// The Joda-Time way of creating a date (the created date is: September the 12., 2004)
		formalPublication.setDate(new LocalDate(2004, 9, 12));
		...
		for (Reference f : list) {
			...
			if (!(((FormalPublication) f).equals(formalPublication) || ((FormalPublication) f)
				.equals(formalPublication2))) {
		...
		FormalPublication formalPublication4 = (FormalPublication) list.get(0);

		// the next two lines are here due to a strange bug of mysql
		// formalPublication4.getWhenInserted().setNanos(0);
		// formalPublication2.getWhenInserted().setNanos(0);

		assertTrue("There is an unexpected formal publication.", formalPublication4.equals(formalPublication2));
	}
	...
	public void testInsertBook() {
		...
		book.setDescription("This book shows you how to develop with " + "the spring framework.");
		book.setVersion("1.0");
		...
		// The Joda-Time way of creating a date (the created date is: June the 21., 2004)
		book.setDate(new LocalDate(2004, 6, 21));
	...
	/**
	 * This test adds a book and keywords. Some of the keywords are related with the book some are not. After that the
	 * book will be get by primary key and compared with the added one. At the end the book will be removed.
	 */
	@Test
	public void testInsertGetRemoveBook() {
		...
		assertEquals("There are not three keywords related with the book.", 3, listKeywords2.size());
		...
		List<Reference> list = service.getReferencesByName("Expert One-on-One J2EE Development without EJB");
		assertEquals("There is still a formal publication with name 'Expert "
			+ "One-on-One J2EE Development without EJB'.", list.size(), 0);
	}

	/**
	 * This test adds a book with three keywords. After saving the book the same book will be get from database. This
	 * book get a new name and one keyword will be replaced by another. Afterwards changes will be saved to database.
	 */
	@Test
	public void testInsertChangeBook() {
		...
		List<Reference> list = service.getReferencesByName("Expert One-on-One J2EE Development without EJB");
		assertEquals("Not one book with name 'Expert One-on-One " + "J2EE Development without EJB'.", 1, list.size());
		...
		List<Reference> emptyList = service.getReferencesByName("Expert One-on-One J2EE " + "Development without EJB");
		assertEquals(
			"Formal publication with name 'Expert One-on-One J2EE " + "Development without EJB' still exists.",
			emptyList.size(), 0);

		List<Reference> list2 = service.getReferencesByName("Springframework - J2EE " + "Development without EJB");
		assertEquals("Not one book with name 'Springframework - J2EE " + "Development without EJB'.", 1, list2.size());
		Book book3 = (Book) list.get(0);
		assertTrue("Books are not equal.", book2.equals(book3));

		Set<Keyword> listKeywords3 = book3.getKeywords();
		assertEquals("There are not three keywords related with the book.", 3, listKeywords3.size());
	...
	public void testGetAllBooks() {
		...
		// book.setDate(new Date(c2.getTimeInMillis()));

		// The Joda-Time way of creating a date (the created date is: May the 7., 2003)
		book.setDate(new LocalDate(2003, 5, 7));
	...
	/**
	 * This test tries out many possible combinations of searching on references.
	 */
	@Test
	public void testSearchReferences() {
		...
		Keyword kUml = createAndSaveKeyword("uml", "Unified modeling language.", dao);
		Keyword kLanguage = createAndSaveKeyword("language", "Program or design language.", dao);
		Keyword kJsp = createAndSaveKeyword("jsp", "Java server pages.", dao);
		Keyword kJava = createAndSaveKeyword("java", "Java program language.", dao);
		Keyword kStruts = createAndSaveKeyword("struts", "Web MVC product.", dao);
		Keyword kDictionary = createAndSaveKeyword("dictionary", "Dictionary.", dao);
		Keyword kJ2ee = createAndSaveKeyword("j2ee", "Java enterprise library.", dao);
		Keyword kIbatis = createAndSaveKeyword("ibatis", "Apache IBatis software.", dao);
		Keyword kDeveloperguide = createAndSaveKeyword("developerguide", "Guide for developers.", dao);
		Keyword kBarcode = createAndSaveKeyword("barcode", "Machine readable string.", dao);
		Keyword k2d = createAndSaveKeyword("2d", "Two dimensions.", dao);
		Keyword kEnglish = createAndSaveKeyword("english", "English language.", dao);
		Keyword kGerman = createAndSaveKeyword("german", "German language.", dao);
		createAndSaveKeyword("zombie", "", dao);

		Book bUmlDistilled = new Book();
		bUmlDistilled.setName("UML Distilled");
		bUmlDistilled.setDescription("A brief guide to the standard object modeling language.");
		...
		bBeginningJsp2.setDescription("Build Web Applications Using Jsp, Java, and Struts.");
		...
		lLeoEngGer.setDescription("An online dictionary to translate German to English and reverse.");
		...
		fSqlMaps2DevGuide.setDescription("This guide shows you how to " + "develop an application using SqlMap 2.0.");
		...
		f2dBarcodePdf417.setDescription("Describse who a " + "PDF417 2D barcode is built-up.");
		...
		assertEquals("There are not seven references for " + "query name='' and description=''.", 7, list.size());

		query = new QueryObject();
		query.addCriteria(ComparisonCriteria.equals("incomplete", false));
		list = service.searchReferences(query);
		assertEquals("There are not three references for query name='' and " + "description='' and incomplete=false.",
			3, list.size());
		...
		for (Reference r : list) {
			...
			if (!(r.equals(bUmlDistilled) || r.equals(lLeoEngGer) || r.equals(f2dBarcodePdf417))) {
		...
		list = service.searchReferences(query);
		assertEquals("There are not four references for " + "query name='2' and description=''.", 4, list.size());
		...
		for (Reference r : list) {
			// r.getWhenInserted().setNanos(0);
			if (!(r.equals(bBeginningJsp2) || r.equals(lJ2eeJsp) || r.equals(fSqlMaps2DevGuide) || r
				.equals(f2dBarcodePdf417))) {
				fail("There was an unexpected reference.");
			}
		}
		...
		list = service.searchReferences(query);
		assertEquals("There are not three references for query name='2' and " + "description='' and incomplete=true.",
			3, list.size());

		for (Reference r : list) {
			// r.getWhenInserted().setNanos(0);
			if (!(r.equals(bBeginningJsp2) || r.equals(lJ2eeJsp) || r.equals(fSqlMaps2DevGuide))) {
				fail("There was an unexpected reference.");
			}
		}

		query = new QueryObject();
		query.addCriteria(LikeCriteria.caseInsensitive("description", "%deVeLop%"));
		list = service.searchReferences(query);
		assertEquals("There are not two references for query name='' and " + "description='deVeLop'.", 2, list.size());
		...
		list = service.searchReferences(query);
		assertEquals("There was not one reference for " + "query name='JAVA' and description='WEB'.", 1, list.size());
		...
		list = service.searchReferences(query);
		assertEquals("Unexpected number of references with keyword java.", 3, list.size());
		...
		query.addCriteria(LikeCriteria.caseInsensitive("description", "%WEB%"));
		list = service.searchReferences(query);
		assertEquals("Unexpected number of references with keyword java and " + "description='WEB'.", 2, list.size());
		...
		query.addCriteria(new IncludeCriteria("keywords", kJava.getKeyAsObject()));
		query.addCriteria(new IncludeCriteria("keywords", kStruts.getKeyAsObject()));

		query.addCriteria(LikeCriteria.caseInsensitive("description", "%WEB%"));
		list = service.searchReferences(query);
		assertEquals("Unexpected number of references with keyword java, " + "keyword struts and description='WEB'.",
			1, list.size());
	 ...
	 * 
	 * @param name
	 *            Is the name of the keyword to create.
	 * @param description
	 *            Is the description of the keyword to create.
	 * @param dao
	 *            Is the dao where to save the created keyword.
	 * @return Returns the created and saved keyword.
	 */
	protected Keyword createAndSaveKeyword(String name, String description, KeywordDao dao) {
		Keyword keyword = new Keyword();
	...
	/**
	 * This test tries to insert an annotation without a content. Because the annotation and its content are written in
	 * two steps, the first step should be redone if the second step fails. This test will be executed on service layer
	...
	public void testIncompleteAnnotationInsertion() {
		...
		try {
			...
			s_logger.debug("Expected exception because content of an " + "annotation can not be null.", e);
		}

		List list = annotationDao.getAnnotationsByAnnotator("Mister Lazy");
		assertEquals("There is still an annotation " + "with annotator 'Mister Lazy' in database.", list.size(), 0);
	}

	/**
	 * This tests generates references and related keywords. In a second step, a service method deletes one reference
	 * and all its connected keywords. Because another reference still is related to some of now-deleted keywords, the
	 * service method should be rolled back.
	 */
	@Test
	public void testMissingKeyword() {
		...
		Keyword kCls = createAndSaveKeyword("cls", "Classics.", dao);
		Keyword kFut = createAndSaveKeyword("fut", "Future.", dao);
		...
		assertEquals("Reference removal was not rolled back.", 2, referenceList.size());

		List<Keyword> keywordList = dao.getAll();
		assertEquals("Keyword removal was not rolled back.", 2, keywordList.size());

		service.deleteReference(bGolding.getKey());
		service.deleteReference(bHuxley.getKey());
		referenceList = service.getAllReferences();
		assertEquals("References not properly removed.", 0, referenceList.size());

		service.deleteKeyword(kCls.getKey());
		service.deleteKeyword(kFut.getKey());
		keywordList = dao.getAll();
		assertEquals("Keywords not properly removed.", 0, keywordList.size());
	}
